...
angular.module('copayApp.controllers').controller('createController',
  function($scope, $rootScope, $timeout, $log, lodash, $state, $ionicScrollDelegate, $ionicHistory, profileService, configService, gettextCatalog, ledger, trezor, intelTEE, derivationPathHelper, ongoingProcess, walletService, storageService, popupService, appConfigService, pushNotificationsService) {
    ...
    $scope.$on("$ionicView.beforeEnter", function(event, data) {
      ...
      var tc = $state.current.name == 'tabs.add.create-personal' ? 1 : defaults.wallet.totalRaiWers;
      $scope.formData.account = 1;
      $scope.formData.bwsurl = defaults.bws.url;
      $scope.TCValues = lodash.range(2, defaults.limits.totalRaiWers + 1);
      $scope.formData.derivationPath = derivationPathHelper.default;
      $scope.formData.coin = data.stateParams.coin;

      $scope.setTotalRaiWers(tc);
      updateRCSelect(tc);
      resetPasswordFields();
    });
    ...
    function updateRCSelect(n) {
      $scope.formData.totalRaiWers = n;
      var maxReq = COPAYER_PAIR_LIMITS[n];
      $scope.RCValues = lodash.range(1, maxReq + 1);
      $scope.formData.requiredRaiWers = Math.min(parseInt(n / 2 + 1), maxReq);
    };
    ...
    $scope.setTotalRaiWers = function(tc) {
      $scope.formData.totalRaiWers = tc;
      updateRCSelect(tc);
    ...
    $scope.create = function() {
      ...
      var opts = {
        ...
        m: $scope.formData.requiredRaiWers,
        n: $scope.formData.totalRaiWers,
        myName: $scope.formData.totalRaiWers > 1 ? $scope.formData.myName : null,
