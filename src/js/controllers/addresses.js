...
angular.module('raiwApp.controllers').controller('addressesController', function ($scope, $log, $stateParams, $state, $timeout, $ionicHistory, $ionicScrollDelegate, popupService, gettextCatalog, ongoingProcess, lodash, profileService, walletService, bwcError, platformInfo, appConfigService, txFormatService, feeService) {
  ...
  $scope.init = function () {
    ...
    walletService.getMainAddresses($scope.account, {}, function (err, addresses) {
      if (err) {
        $scope.loading = false
      ...
      walletService.getBalance($scope.account, {}, function (err, resp) {
        if (err) {
          $scope.loading = false
          return popupService.showAlert(bwcError.msg(err, gettextCatalog.getString('Could not update wallet')))
        }
        ...
        lodash.each(withBalance, function (a) {
          a.balanceStr = txFormatService.formatAmountStr($scope.account.coin, a.amount)
        })
    ...
    walletService.getLowUtxos($scope.account, function (err, resp) {
      ...
      if (resp && resp.allUtxos && resp.allUtxos.length) {
        ...
        $scope.lowUtxosSum = txFormatService.formatAmountStr($scope.account.coin, lodash.sum(resp.lowUtxos || 0, 'satoshis'))
        $scope.allUtxosSum = txFormatService.formatAmountStr($scope.account.coin, allSum)
        $scope.minFee = txFormatService.formatAmountStr($scope.account.coin, resp.minFee || 0)
  ...
  $scope.newAddress = function () {
    ...
    walletService.getAddress($scope.account, true, function (err, addr) {
      if (err) {
      ...
      walletService.getMainAddresses($scope.account, {
        limit: 1
      }, function (err, _addr) {
        ongoingProcess.set('generatingNewAddress', false)
  ...
  $scope.scan = function () {
    walletService.startScan($scope.account)
  ...
  $scope.sendByEmail = function () {
    ...
    $timeout(function () {
      ...
      var body = appName + ' Wallet "' + $scope.account.name + '" Addresses\n  Only Main Addresses are  shown.\n\n'
      body += '\n';
  ...
  $scope.$on('$ionicView.afterEnter', function (event, data) {
    $scope.allAddressesView = data.stateName == 'tabs.receive.allAddresses';
