...
angular.module('canoeApp.controllers').controller('txpDetailsController', function ($scope, $rootScope, $timeout, $interval, $log, ongoingProcess, platformInfo, $ionicScrollDelegate, txFormatService, gettextCatalog, lodash, walletService, popupService, $ionicHistory) {
    ...
    popupService.showConfirm(title, msg, null, null, function (res) {
      if (res) {
        ...
        walletService.reject($scope.account, $scope.tx, function (err, txpr) {
          if (err)            { return setError(err, gettextCatalog.getString('Could not reject payment'))}
  ...
  var updateTxInfo = function (eventName) {
    $scope.account.getTx($scope.tx.id, function (err, tx) {
      ...
      if (!action && tx.status == 'pending')        { $scope.tx.pendingForUs = true}
